...
#
# Final unified version with resume capability, robust error handling,
# the most advanced prompt, and post-processing deduplication.
# -- MODIFIED TO INCLUDE 'MOMENTO DO JOGO' IN THE FINAL DETAILED REPORT --
...
class Config:
    ...
    BRANDS_TO_EXCLUDE: Set[str] = {
        ...
        'youtube premium', 'paulistão sicredi',
        
        # People Names & Non-Sponsor Entities
        'neymar', 'neymar jr', 'guilherme', 'gabriel medina', 'bruninho', 
        'virgínia', 'zé felipe', 'santos', 'botafogo', 'santos', 'corinthians', 'palmeiras',
        'alexandre jesus'
    }

    FORMATS_TO_EXCLUDE: Set[str] = {
        'None'
    }
    
    MAX_FRAME_DIMENSION: int = 512
    DEDUPLICATION_WINDOW_SECONDS: int = 12 # A more robust window
    MAIN_TEMP_DIR: Path = Path("analysis_temp")

    @classmethod
    def setup_directories(cls):
        """Prepares temporary directories, preserving existing files for resume capability."""
...
def download_and_split_media(video_url: str, video_id: str, total_duration_seconds: int) -> bool:
    """
    Downloads, extracts audio, and segments the media into chunks.
    This function contains its own checks to avoid re-downloading/re-chunking.
    """
    local_video_path = Path(f"video_{video_id}.mp4")
    local_audio_path = Path(f"audio_{video_id}.mp3")
    yt_dlp_common = ['yt-dlp', '--geo-bypass-country', 'BR', '--no-check-certificate', '--retries', 'infinite', '--quiet']
    if Config.COOKIE_FILE_PATH and Config.COOKIE_FILE_PATH.exists():
        yt_dlp_common.extend(['--cookies', str(Config.COOKIE_FILE_PATH)])
    
    try:
        # Internal Check 1: Full video download
        if not local_video_path.exists():
            logging.info(f"Downloading full video to '{local_video_path}'...")
            ...
            logging.info(f"Full video '{local_video_path}' already exists. Skipping download.")
        
        # Internal Check 2: Full audio extraction
        if not local_audio_path.exists():
            logging.info(f"Extracting full audio to '{local_audio_path}'...")
            cmd = yt_dlp_common + ['-x', '--audio-format', 'mp3', '-o', str(local_audio_path), video_url]
            subprocess.run(cmd, check=True, capture_output=True, text=True)
        else:
            logging.info(f"Full audio '{local_audio_path}' already exists. Skipping extraction.")

        # Internal Check 3: Chunk existence
        if total_duration_seconds > 0:
            ...
            if existing_video_chunks >= expected_num_chunks and existing_audio_chunks >= expected_num_chunks:
                ...
                logging.info(f"Chunking incomplete (Expected: {expected_num_chunks}, Found Video: {existing_video_chunks}, Found Audio: {existing_audio_chunks}). Re-segmenting.")
...
# --- CORE ANALYSIS LOGIC ---
# --- PASTE THIS ENTIRE FUNCTION INTO analysis_pipeline.py, REPLACING THE OLD ONE ---
...
5.  **DISTINÇÃO CRÍTICA: Uma 'marca' é uma empresa, produto ou serviço (ex: PagBank, Clear, FIAT, McDonald's, Rexona, Jeep, Benegrip, Casas Bahia, Claro, Buscopan). Nomes de pessoas (jogadores, celebridades, etc., como Neymar, Virgínia) NÃO SÃO MARCAS e não devem ser incluídos na resposta. Além disso, se um anúncio mencionar uma linha de produtos junto com a marca (ex: 'Híbridos FIAT'), sua tarefa é extrair e reportar a marca principal ('FIAT').**
    * **Referência Visual das Marcas:** Para auxiliar na identificação precisa, considere as seguintes características visuais das marcas mais frequentes:
        ...
        * **McDonalds:** O logo do McDonald's é o icônico "M" dourado, estilizado para se assemelhar a arcos, representando os "Arcos Dourados" da marca.
...
def create_detailed_report(all_detections: List[Dict], video_id: str, video_title: str, video_upload_datetime: datetime.datetime, output_path: Path) -> None:
    ...
    for ad in all_detections:
        ...
        report_data.append({
            ...
            'Formato / Contexto': ad.get('Resumo do Anúncio', 'N/A').replace('Contexto: ', '') if is_audio else ad.get('Formato do Anúncio', 'N/A'), 
...
def cleanup_temporary_files():
    logging.info("Cleaning up temporary analysis directory...")
    try:
        if Config.MAIN_TEMP_DIR.exists(): shutil.rmtree(Config.MAIN_TEMP_DIR)
    except Exception as e:
        logging.error(f"Could not complete temporary file cleanup: {e}")

def cleanup_cached_media(video_id: str):
    logging.info(f"Cleaning up cached media for video_id: {video_id}...")
    try:
        for p in Path('.').glob(f"video_{video_id}.*"): p.unlink(); logging.info(f"Removed cached video file: {p}")
        for p in Path('.').glob(f"audio_{video_id}.*"): p.unlink(); logging.info(f"Removed cached audio file: {p}")
    except Exception as e:
        logging.error(f"Could not complete cached media cleanup: {e}")

def force_clear_specific_files_from_trash(video_id: str):
    if sys.platform != "linux": return
    logging.info("Linux system detected. Checking trash for leftover media files...")
    trash_path = Path.home() / ".local/share/Trash/files"
    if not trash_path.is_dir(): return
    files_cleaned = False
    for pattern in [f"video_{video_id}.*", f"audio_{video_id}.*"]:
        for p in trash_path.glob(pattern):
            try:
                subprocess.run(['rm', '-f', str(p)], check=True)
                logging.info(f"FORCE-DELETED from trash: {p.name}")
                files_cleaned = True
            except Exception as e:
                logging.error(f"Failed to force-delete {p.name} from trash: {e}")
    if files_cleaned: logging.info("Forced cleanup from trash completed.")

# --- ANALYSIS ENGINE (SIMPLIFIED) ---
def _run_analysis_engine(youtube_url: str, metadata: tuple) -> bool:
    """
    This function now assumes it's only called when work needs to be done.
    It handles the download, chunking, analysis, and report creation.
    """
    video_id, video_title, duration_seconds, video_upload_datetime = metadata

    # STAGE 1: Prepare Media (Download/Chunk)
    if not download_and_split_media(youtube_url, video_id, duration_seconds):
        logging.error("Failed to prepare media. Aborting analysis.")
        return False
    
    # STAGE 2: Check for and Run Analysis on Unprocessed Chunks
    ...
    num_total_chunks = min(len(video_chunks), len(audio_chunks))
    tasks_to_run = []
    for i in range(num_total_chunks):
        if i not in completed_chunks:
            tasks_to_run.append((i, video_chunks[i], audio_chunks[i]))

    if not tasks_to_run and num_total_chunks > 0:
        logging.info("All chunks have been processed previously. Moving directly to report generation.")
    elif tasks_to_run:
        logging.info(f"Found {len(completed_chunks)} completed chunks. Resuming analysis for {len(tasks_to_run)} remaining chunks.")
        ...
        logging.error("No media chunks were created. Cannot proceed with analysis.")
        return False

    # STAGE 3: Aggregate Results and Generate Reports
    all_detections = []
    ...
    output_dir = Path("output_reports_v3")
...
# --- MAIN EXECUTION BLOCK (GATEKEEPER) ---
def main(url: str) -> Tuple[bool, Optional[str]]:
    """Main function to run the full analysis pipeline for a single URL."""
    success = False
    video_id = None
    ...
    if not metadata:
        logging.error(f"Could not retrieve metadata for {url}. Cannot proceed.")
        return False, None
    
    video_id, video_title, _, video_upload_datetime = metadata

    # --- GATEKEEPER CHECK: Use video_id for robust identification ---
    date_prefix = video_upload_datetime.strftime('%Y-%m-%d')
    output_dir = Path("output_reports_v3")
    summary_report_path = output_dir / f"{date_prefix}_summary_report_{video_id}.csv"
    detailed_report_path = output_dir / f"{date_prefix}_detailed_log_{video_id}.csv"
    ...
    if summary_report_path.exists() and detailed_report_path.exists():
        logging.info(f"SUCCESS: Final reports for video ID '{video_id}' already exist. No action needed.")
        return True, video_id # Exit gracefully
    # --- END OF GATEKEEPER CHECK ---

    try:
        # Set multiprocessing start method for compatibility across systems
    ...
    try:
        # Only call the engine if reports are missing
        logging.info(f"Final reports not found. Starting analysis for video ID '{video_id}' (Title: {video_title}).")
        success = _run_analysis_engine(url, metadata)
    except Exception as e:
        logging.critical(f"An unhandled error occurred during the analysis of {url}. Error: {e}", exc_info=True)
        success = False
    finally:
        logging.info(f"Finished processing for video_id: {video_id}. Final Status: {'Success' if success else 'Failed'}")

    if success:
        logging.info("Analysis completed successfully. Cleaning up temporary files and cached media.")
        cleanup_cached_media(video_id)
        cleanup_temporary_files()
    else:
        logging.warning("Analysis did not complete successfully. Temporary files and cached media are being kept for resume.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # vvvv CHANGE THE URL ON THIS LINE FOR DIRECT RUNS vvvv
        video_url = "https://www.youtube.com/watch?v=FGNuB6eMSEo"
